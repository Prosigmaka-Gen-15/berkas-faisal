...
function CartItem() {
  ...
  return (
    <div className="flex flex-col justify-between space-y-4 min-h-screen">
      <Header />
      <div>
        ...
        {cart.cartItems.length === 0 ? (
          ...
          <div className="flex flex-col items-center gap-4 w-full ">
            {cart.cartItems?.map((cartItem) => (
              <div
                className="flex items-center border-lg rounded-lg py-24 w-2/4 h-40 bg-gray-200"
                key={cartItem.id}
              >
                <img
                  src={getImgUrl(cartItem.image)}
                  alt="title"
                  className="w-32 h-32 object-cover p-4"
                />
                <div className="ml-4 flex-grow">
                  <h2 className="text-lg font-semibold">{cartItem.name}</h2>
                  <p className="text-gray-600">Rp.{cartItem.price}</p>
                </div>
